...
export default function PatientSharingModal({ isOpen, onClose, reportName }: PatientSharingModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <div className="fixed inset-0 bg-[#2C3E3E]/60 backdrop-blur-sm" onClick={onClose}></div>
            <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                <div className="bg-[#F6F7F5] p-8 border-b border-[#E2E8F0] flex justify-between items-center">
                    <div>
                        <h2 className="text-2xl font-black text-[#2C3E3E] tracking-tight">Share Report</h2>
                ...
                <div className="p-8">
                    {isSuccess ? (
                        <div className="text-center py-12 space-y-4 animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <h3 className="text-xl font-black text-[#2C3E3E]">Success!</h3>
                        ...
                        <form onSubmit={handleShare} className="space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-6 py-4 bg-[#F6F7F5] border-transparent rounded-2xl outline-none focus:bg-white focus:border-[#8FB9A8] border-2 transition-all font-bold text-sm text-[#2C3E3E]"
                                />
                            </div>
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full px-6 py-4 bg-[#F6F7F5] border-transparent rounded-2xl outline-none focus:bg-white focus:border-[#8FB9A8] border-2 transition-all font-bold text-sm text-[#2C3E3E] resize-none"
                                ></textarea>
                            </div>
                            <button
                                type="submit"
                                disabled={isSharing}
                                className="w-full bg-[#4F6F6F] text-white py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-lg hover:bg-[#2C3E3E] transition-all flex items-center justify-center space-x-3 disabled:opacity-50"
                            >
                                {isSharing ? (
                                    <>
