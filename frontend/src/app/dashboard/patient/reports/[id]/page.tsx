...
export default function PatientReportViewerPage() {
    ...
    if (error || !report) return (
        <div className="text-center p-20 bg-white rounded-[40px] shadow-sm border border-[#E2E8F0]">
            ...
            <h3 className="text-xl font-black text-[#2C3E3E] mb-2">{error ?? 'Report not found'}</h3>
            <p className="text-[#6B7280] font-medium mb-8">We could not retrieve the requested laboratory report.</p>
            <button onClick={() => router.back()} className="bg-[#4F6F6F] text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#2C3E3E] transition-all">Go Back</button>
        </div>
    );
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-1.5">
                        ...
                        <h1 className="text-2xl font-black text-[#2C3E3E] tracking-tight">{report.reportName}</h1>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <a href={fullFileUrl} download className="flex items-center gap-2 bg-[#4F6F6F] text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-[#2C3E3E] transition-all shadow-md">
            ...
            <div className="flex items-center gap-1 bg-white border border-[#E2E8F0] p-1 rounded-2xl w-fit shadow-sm">
                {TABS.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        className={`px-5 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === tab.id ? 'bg-[#4F6F6F] text-white shadow-md' : 'text-[#6B7280] hover:text-[#2C3E3E]'}`}>
                        {tab.label}
                    </button>
                ))}
            </div>
                            ...
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        ...
                                        <span className="text-[10px] font-bold text-emerald-600">Processed by HealthScan AI</span>
                                    </div>
                                </div>
                            </div>
                            <div
                    ...
                    {biomarkerRows.length > 0 && !isProcessing && (
                        <div className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm overflow-hidden">
                            <div className="p-6 border-b border-[#F6F7F5]">
                                <h3 className="text-lg font-black text-[#2C3E3E]">Biomarker Results</h3>
                                    ...
                                    <tbody className="divide-y divide-[#F6F7F5]">
                                        {biomarkerRows.map(({ key, val, norm, status }) => (
                                            <tr key={key} className="hover:bg-[#F6F7F5] transition-colors">
                                                <td className="px-6 py-4 font-black text-[#2C3E3E] text-sm capitalize">{key}</td>
                                                <td className="px-6 py-4">
                                                    <span className={`font-black text-lg ${status === 'normal' ? 'text-emerald-600' : status === 'low' ? 'text-amber-600' : status === 'high' ? 'text-rose-600' : 'text-[#2C3E3E]'}`}>
                    ...
                    {report.doctorComment && !isProcessing && (
                        <div className="bg-white p-8 rounded-3xl border border-[#E2E8F0] shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-lg font-black text-[#2C3E3E]">Doctor&apos;s Clinical Review</h3>
                            </div>
