...
const ROLE_COLORS: Record<string, string> = {
  patient: 'bg-[#F6F7F5] text-[#4F6F6F] border-[#E2E8F0]',
  doctor: 'bg-emerald-50 text-emerald-700 border-emerald-100',
  pathology: 'bg-[#8FB9A8]/10 text-purple-700 border-purple-100',
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-black text-[#2C3E3E] tracking-tight">System Administration</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: 'Total Users', value: stats.totalUsers, color: 'text-[#2C3E3E]' },
          { label: 'Total Reports', value: stats.totalReports, color: 'text-[#4F6F6F]' },
          { label: 'Patients', value: stats.patients, color: 'text-[#4F6F6F]' },
          { label: 'Doctors', value: stats.doctors, color: 'text-emerald-600' },
          { label: 'Labs / Admins', value: stats.labs, color: 'text-[#8FB9A8]' },
        ].map(s => (
      ...
      <div className="flex items-center gap-1 bg-[#F6F7F5] p-1 rounded-2xl w-fit">
        {(['users', 'reports', 'roles'] as Tab[]).map(t => (
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-black capitalize transition-all ${tab === t ? 'bg-white text-[#2C3E3E] shadow-sm' : 'text-[#6B7280] hover:text-[#2C3E3E]'}`}
          >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm overflow-hidden">
          <div className="p-6 border-b border-[#F6F7F5] flex items-center justify-between">
            <h2 className="text-lg font-black text-[#2C3E3E]">All Users ({filteredUsers.length})</h2>
          </div>
                    ...
                    <tr key={u._id} className="hover:bg-[#F6F7F5] transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-black text-[#2C3E3E]">{u.name}</span>
                        </div>
                      </td>
        ...
        <div className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm overflow-hidden">
          <div className="p-6 border-b border-[#F6F7F5]">
            <h2 className="text-lg font-black text-[#2C3E3E]">All Reports ({filteredReports.length})</h2>
          </div>
                ...
                <tbody className="divide-y divide-[#F6F7F5]">
                  {filteredReports.map(r => (
                    <tr key={r._id} className="hover:bg-[#F6F7F5] transition-colors">
                      <td className="px-6 py-4 text-sm font-black text-[#2C3E3E]">{r.reportName}</td>
            ...
            {[
              ...
              <div key={r.role} className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-black text-[#2C3E3E]">{r.role}</h3>
